...
await parentw.sendMessage(m.chat, { text: '🚩 Sub-Bot eliminado.' }, m)
} catch(err) {
if (err.code === 'ENOENT' && err.path === `./${jadi}/${uniqid}`) {
await parentw.sendMessage(m.chat, { text: "🍟 No cuentas con ninguna sesión de Sub-Bot." }, m)
} else {
await m.react(error)
}}}
handler.help = ['deletebot']
handler.tags = ['serbot']
handler.command = ['stop', 'delsession', 'eliminarsession']
