...
await conn.sendMini(m.chat, packname, dev, wel, img, img, channel, m)
  }

  if (chat.welcome && m.messageStubType == 28) {
   let bye = `┌─★ Nixie \n│「 Adios 」\n└┬★ 「 @${m.messageStubParameters[0].split`@`[0]} 」\n   │ Se Fue\n   │ Ojala no vuelva\n   └───────────────┈ ⳹`
await conn.sendMini(m.chat, packname, dev, bye, img2, img2, channel, m)
  }

  if (chat.welcome && m.messageStubType == 32) {
    let kick = `┌─★ Nixie \n│「 Adios 」\n└┬★ 「 @${m.messageStubParameters[0].split`@`[0]} 」\n   │ se fue\n   │ Ojala no regrese\n   └───────────────┈ ⳹`
await conn.sendMini(m.chat, packname, dev, kick, img2, img2, channel, m)
}}
